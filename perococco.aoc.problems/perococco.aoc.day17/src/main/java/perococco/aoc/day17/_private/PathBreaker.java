...
public class PathBreaker {
    ...
    private class Context {
        ...
        private MovementRules createRules() {
            final ImmutableMap<MovementFunction,String> routines = Arrays.stream(orders, 0, orderIdx)
                                                                         .collect(ImmutableMap.toImmutableMap(o->o.function,
                                                          Order::toRoutine));

            return new MovementRules(ImmutableList.copyOf(functions1), routines);
        }
