...
import perococco.aoc.computer.io._private.input.ControllerInputPort;
import perococco.aoc.computer.io._private.input.InputPortFromList;
import perococco.aoc.computer.io._private.input.InputPortWithSupplier;
import perococco.aoc.computer.io._private.input.NoInputPort;
...
public interface ProgramIO<I, O> extends ProgramIOAccessors<I,O> {
    ...
    static OutputPartBuilder<Nil> fromList(@NonNull String... values) {
        return new OutputPartBuilder<>(new InputPortFromList(ImmutableList.copyOf(values)));
    }

    @NonNull
    static <T> OutputPartBuilder<Nil> fromList(@NonNull InputMultiTransformer<T> transformer, @NonNull T... values) {
        final ImmutableList<String> strs = Arrays.stream(values).flatMap(r -> transformer.apply(r).stream())
